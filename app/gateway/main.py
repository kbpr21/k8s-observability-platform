...
@app.get("/")
async def read_root():
    return {"service": "gateway", "request_id": request_id_var.get()}


@app.get("/order")
async def create_order():
    req_id = request_id_var.get()
    logger.info(f"Forwarding order to {ORDERS_SERVICE_URL}")
    try:
        response = await client.post(
            ORDERS_SERVICE_URL,
            headers={"X-Request-ID": req_id},
            json={"item": "premium_membership"},
        )
        logger.info(f"Orders responded with status {response.status_code}")
        return JSONResponse(status_code=response.status_code, content=response.json())
    except httpx.HTTPError as exc:
        logger.error(f"Failed to reach orders service: {exc}")
        return JSONResponse(
            status_code=502,
            content={"detail": f"Failed to contact orders service: {exc}", "request_id": req_id},
        )
