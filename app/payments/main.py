...
async def charge_payment(payload: dict):
    ...
    if fault_active:
        ...
        return JSONResponse(
            status_code=500,
            content={"status": "error", "message": "Simulated payment failure", "order_id": order_id},
        )
...
async def toggle_fault(payload: dict):
    global fault_active
    fault_active = payload.get("active", False)
    logger.warning(f"Fault state changed: fault_active={fault_active}")
